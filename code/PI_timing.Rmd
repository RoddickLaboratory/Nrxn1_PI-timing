...
During the final block of testing, all genotypes made a similar number of total nose pokes per session (`r report_anova(pokes_aov)`).
This was true when examining both nose pokes made while the discriminative stimulus was on (`r report_anova(stim_pokes_aov)`), and during the inter trial intervals (`r report_anova(ITI_pokes_aov)`).
